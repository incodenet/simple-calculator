...
function initialize() {
  ...
  (function selectTheme(){
    ...
    for (var i = 0; i < themeBtn.length; i++) {
      if(themeBtn[i].classList.contains("selected")) {
        var themeName = themeBtn[i].getAttribute("data-theme");
  ...
  (function processOperations() {
    ...
    for (var i = 0; i < keys.length; i++) {
      keys[i].addEventListener("click", function(e) {
        ...
        switch(type) {
          case "action":
            respondToAction(type, role, null);
            break;

          case "number":
            keyControl.classList.remove("operator-exists");
            respondToAction(type, role, parsedSymbols);
            break;

          case "symbol":
            keyControl.classList.remove("operator-exists");
            respondToAction(type, role, symbols);
            break;
  ...
  function respondToAction(type, role, value){
    switch (role) {
      case "cancel":
        cancel(type);
        break;
